...
public class SafeNode {
	...
	private static VarAccess threadsAc() {return new VarAccess("threads");}
	...
	private static TypeAccess arrayLType() {return new TypeAccess("ArrayList");}
	private static ThisAccess thisAc() {return new ThisAccess("this");}
	
	
	/**
	 * Class that creates the SafeNode Class in the file with the following methods.
	 * 
  ...
  public synchronized boolean existsThread(Thread thread) {
	...
	createAndAddClassDecl(List<TypeDecl> list){
		...
		list.add(new ClassDecl(new Modifiers(),"SafeNode",opt,
				listaAccess,listaBodyDecl));
		return list;
	}
	...
	private static void createFields(List<BodyDecl> retorno) {
		...
		FieldDeclaration threads = new FieldDeclaration(
				listaModifiers,arrayLType(),"threads");

		FieldDeclaration catchExceptions = new FieldDeclaration(
				listaModifiers,arrayLType(),"catchExceptions");
		...
		parametersIndex.addChild(new ClassInstanceExpr(
				new TypeAccess("AtomicInteger"),parametersNewAtomicInteger));
		FieldDeclaration index = new FieldDeclaration(listaModifiers,
				new TypeAccess("AtomicInteger"),"index",parametersIndex);
	...
	private static void createConstrutor(List<BodyDecl> retorno) {
		...
		ExprStmt exprSetThreads = new ExprStmt(
				new AssignSimpleExpr(new Dot(thisAc(),threadsAc())
				,new ClassInstanceExpr(arrayLType(),new List<Stmt>())));

		Block block = new Block();
		block.addStmt(exprSetThreads);

		ConstructorDecl contrutor = new ConstructorDecl(listaModifiers,
				"SafeNode",new List<ParameterDeclaration>(),
	...
	private static void createAddThread(List<BodyDecl> retorno){
		...
		ExprStmt addThreadExprStmt = new ExprStmt(new Dot(thisAc(),
				new Dot(threadsAc(),new MethodAccess("add",
						new List<Expr>().add(threadAc())))));

		ExprStmt incrementAndGetExprStmt = new ExprStmt(new Dot(
				thisAc(),new Dot(new VarAccess("index"),
						new MethodAccess("incrementAndGet",
								new List<Expr>()))));
		...
		parameters.add(new ParameterDeclaration(threadType(),"thread"));

		MethodDecl method = new MethodDecl(listaModifiers,primitiveAccess,
				"addThread",parameters,new List<Access>(),opt);
	...
	private static void createTryWakeUp(List<BodyDecl> retorno) {
		...
		ExprStmt removeThreadExprStmt = new ExprStmt(new Dot(
				thisAc(),new Dot(threadsAc(),new MethodAccess(
						"remove",new List<Expr>().add(threadAc())))));

		ExprStmt decrementAndGetExprStmt = new ExprStmt(new Dot(
				thisAc(),new Dot(new VarAccess("index"),
						new MethodAccess("decrementAndGet",
								new List<Expr>()))));

		ExprStmt notifyAllExprStmt = new ExprStmt(new MethodAccess(
				"notifyAll",new List<Expr>()));
		...
		MethodDecl method = new MethodDecl(listaModifiers,primitiveAccess,
				"tryWakeUp",parameters,new List<Access>(),opt);
	...
	private static void createAddException(List<BodyDecl> retorno) {
		...
		ExprStmt addExceptionExprStmt = new ExprStmt(new Dot(
				thisAc(),new Dot(new VarAccess("catchExceptions"),
						new MethodAccess("add",new List<Expr>()
								.add(new VarAccess("excep"))))));

		IfStmt ifStmt = new IfStmt(new EQExpr(new Dot(thisAc(),
				new VarAccess("catchExceptions")),new NullLiteral("null")),
				new ExprStmt(new AssignSimpleExpr(new Dot(thisAc(),
						new VarAccess("catchExceptions")),new ClassInstanceExpr(
								arrayLType(),new List<Expr>()))));
		...
		parameters.add(new ParameterDeclaration(
				new TypeAccess("Exception"),"excep"));

		MethodDecl method = new MethodDecl(listaModifiers,primitiveAccess,
				"addException",parameters,new List<Access>(),opt);
	...
	private static void createSyncUp(List<BodyDecl> retorno) {
		...
		blockTry.addStmt(new ExprStmt(new MethodAccess("wait",
				new List<Expr>())));

		Block blockCatch = new Block();
		blockCatch.addStmt(new ExprStmt(new Dot(new VarAccess("e"),
				new MethodAccess("printStackTrace",new List<Expr>()))));

		BasicCatch catchStmt = new BasicCatch(new ParameterDeclaration(
				new TypeAccess("InterruptedException"),"e"),blockCatch);

		TryStmt tryStmt = new TryStmt(blockTry,
				new List<CatchClause>().add(catchStmt),new Opt<Block>());

		WhileStmt whileStmt = new WhileStmt(new GTExpr(new Dot(
				new VarAccess("index"),new MethodAccess("get",
						new List<Expr>())),new IntegerLiteral(0)),tryStmt);
		...
		MethodDecl method = new MethodDecl(listaModifiers,primitiveAccess,
				"syncUp",parameters,new List<Access>(),opt);
	...
	private static void createBreakUp(List<BodyDecl> retorno) {
		...
		VariableDeclaration variableFor = new VariableDeclaration(
				new PrimitiveTypeAccess("int"),"i",new IntegerLiteral(0));
		...
		LTExpr ltExpr = new LTExpr(new VarAccess("i"),new Dot(
				threadsAc(),new MethodAccess("size",new List<Expr>())));
		...
		CastExpr castExpr = new CastExpr(threadType(),new Dot(threadsAc(),
				new MethodAccess("get",listExpr)));

		ExprStmt exprStmtForStmt = new ExprStmt(new Dot(new ParExpr(castExpr),
				new MethodAccess("interrupt",new List<Expr>())));
		blockFor.addStmt(exprStmtForStmt);

		ForStmt forStmt = new ForStmt(listVariableInitFor,listCondictionFor,
				listDoFor,blockFor);
		...
		MethodDecl method = new MethodDecl(listaModifiers,primitiveAccess,
				"breakUp",parameters,new List<Access>(),opt);
	...
	private static void createExistsThread(List<BodyDecl> retorno) {
		...
		parametersIfExpr.add(threadAc());
		Block blockIf = new Block();
		blockIf.addStmt(new ReturnStmt(
				new BooleanLiteral("true")));

		IfStmt ifStmt = new IfStmt(new Dot(threadsAc(),new MethodAccess(
				"contains",parametersIfExpr)),blockIf);
		...
		MethodDecl method = new MethodDecl(listaModifiers,primitiveAccess,
				"existsThread",parameters,new List<Access>(),opt);
